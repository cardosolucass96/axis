...
/**
 * Obtém a semana atual baseada na data de hoje
 * Retorna a string no formato "MÊS SemN (dia-dia)"
 */
export function getCurrentWeek(month?: string, year?: number): string | null {
  const now = new Date();
  const currentMonth = month || monthNames[now.getMonth()];
  ...
  for (const week of weeks) {
    const range = extractDayRange(week);
    if (range && currentDay >= range.start && currentDay <= range.end) {
      return week;
    }
  }
  
  // Se não encontrou (pode ser mês diferente), retornar a primeira semana
...
export function isSameWeek(week1: string, week2: string): boolean {
  ...
  // Se ambos têm intervalo, comparar intervalos
  if (range1 && range2) {
    return range1.start === range2.start && range1.end === range2.end;
  }
  
  // Se ambos têm número, comparar números
...
export function distributeMonthlyTarget(
  ...
  // Para percentuais, a meta é a mesma para todas as semanas
  if (unit === 'percent') {
    weeks.forEach(week => {
  ...
  // Para outros tipos, distribuir proporcionalmente aos dias
  weeks.forEach(week => {
...
export function recalculateRemainingWeeklyTargets(
  ...
  // Para percentuais, manter meta fixa
  if (unit === 'percent') {
    weeks.forEach(week => {
      result[week] = monthlyTarget;
    });
  ...
  // Calcular total já realizado nas semanas concluídas
  let totalRealized = 0;
  ...
  // Meta restante para distribuir
  const remainingTarget = Math.max(0, monthlyTarget - totalRealized);

  // Calcular dias restantes (semanas não concluídas)
  let remainingDays = 0;
  weeks.forEach(week => {
  ...
  // Distribuir meta restante proporcionalmente
  weeks.forEach(week => {
    const isCompleted = completedWeeks.some(cw => isSameWeek(cw, week));
    const entry = entries.find(e => isSameWeek(e.week, week));
    
    if (isCompleted && entry) {
      // Semana concluída: manter meta original
      result[week] = entry.target;
    } else if (remainingDays > 0) {
      // Semana pendente: redistribuir proporcionalmente
      const days = getDaysInWeek(week);
